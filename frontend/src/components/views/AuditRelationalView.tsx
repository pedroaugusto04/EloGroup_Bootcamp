...
export const AuditRelationalView = () => {
  ...
  if (loading) {
    return <div className="text-[#5e6270] dark:text-[#71717a] text-sm">Carregando dados de auditoria...</div>;
  }
    ...
    <div className="space-y-6 view-enter">
      ...
      <div className="space-y-2.5">
        {(relData?.audit_findings || []).map((finding, idx) => (
          <div
            key={idx}
            className="p-3 sm:p-3.5 rounded-lg bg-[#ffffff] dark:bg-[#131126] border border-[#e6e5f0] dark:border-[#262046] flex items-start gap-2.5 sm:gap-3 text-xs shadow-sm"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1">
                <span className="font-semibold text-[#131920] dark:text-[#f4f4f5] text-xs truncate">
                  {finding.dimension}
                </span>
                <span
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 my-1.5 text-[11px] font-mono bg-[#f8f7fc] dark:bg-[#181530] p-2 rounded border border-[#e6e5f0] dark:border-[#262046]">
                <div>
                  <span className="text-[#8e92a0] dark:text-[#71717a] block">Base Vendas:</span>
                  <span className="text-[#131920] dark:text-[#d4d4d8] font-medium">{finding.erp_coverage}</span>
                </div>
                <div>
                  <span className="text-[#8e92a0] dark:text-[#71717a] block">Outras Bases:</span>
                  <span className="text-[#131920] dark:text-[#d4d4d8] font-medium">{finding.external_coverage}</span>
                </div>
              </div>

              <p className="text-[11px] text-[#5e6270] dark:text-[#a1a1aa] mt-1 font-sans leading-relaxed">
                <strong className="text-[#131920] dark:text-[#e4e4e7]">Impacto Analítico:</strong> {finding.impact}
              </p>
            </div>
          </div>
        ))}
        ...
        <div className="h-64 sm:h-72 w-full mt-2">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={relData?.mkt_vs_sales || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="investimento_mkt" name="Invest. Mídia (R$)" fill={isDark ? '#94a3b8' : '#cbd5e1'} radius={[3, 3, 0, 0]} />
              <Bar dataKey="receita_declarada_mkt" name="Receita Mídia (R$)" fill="#f59e0b" radius={[3, 3, 0, 0]} />
              <Bar dataKey="receita_liquida_real" name="Receita Real (R$)" fill={isDark ? '#8575ff' : '#4200db'} radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
