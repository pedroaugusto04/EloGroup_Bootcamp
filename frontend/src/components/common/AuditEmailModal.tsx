...
export const AuditEmailModal: React.FC<AuditEmailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="relative w-full max-w-lg bg-[#ffffff] dark:bg-[#131126] border border-[#e6e5f0] dark:border-[#262046] rounded-xl shadow-2xl overflow-hidden transition-colors">
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-[#e6e5f0] dark:border-[#262046] bg-[#f8f7fc] dark:bg-[#0d0b1a] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-[#e8e6ff] dark:bg-[#4200db]/20 border border-[#c4b8ff] dark:border-[#8575ff]/40 flex items-center justify-center text-[#4200db] dark:text-[#8575ff]">
          ...
          <button
            onClick={onClose}
            disabled={loading}
            className="p-1.5 rounded-lg text-[#5e6270] dark:text-[#a1a1aa] hover:text-[#131920] dark:hover:text-[#f4f4f5] hover:bg-[#f3f2f8] dark:hover:bg-[#181530] transition-colors disabled:opacity-50"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-5 space-y-4">
          {!result ? (
              ...
              <div className="space-y-3 pt-1">
                <div>
                  ...
                  <select
                    ...
                    disabled={loading}
                    className="w-full text-xs bg-[#ffffff] dark:bg-[#181530] border border-[#e6e5f0] dark:border-[#262046] rounded-lg px-3 py-2 text-[#131920] dark:text-[#f4f4f5] focus:outline-none focus:border-[#4200db] dark:focus:border-[#8575ff]"
                  >
                    {periods.map(period => (
                ...
                <div className="flex items-center gap-2 pt-1">
                  <input
                    ...
                    disabled={loading}
                    className="rounded border-[#e6e5f0] dark:border-[#262046] bg-[#ffffff] dark:bg-[#181530] text-[#4200db] dark:text-[#8575ff] focus:ring-0"
                  />
                  <label htmlFor="sendEmailCheckbox" className="text-xs text-[#131920] dark:text-[#d4d4d8] cursor-pointer select-none">
                    Enviar e-mail
                  </label>
                </div>
              </div>
            ...
            <div className="space-y-3.5 py-1">
              <div className={`p-3.5 rounded-lg text-xs flex items-start gap-2.5 ${result.analysis_success ? 'bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 border border-rose-500/30 text-rose-600 dark:text-rose-400'}`}>
              ...
              <div className="p-3 bg-[#f8f7fc] dark:bg-[#181530] rounded-lg border border-[#e6e5f0] dark:border-[#262046] text-xs space-y-1.5 font-mono">
                <div className="flex justify-between text-[#5e6270] dark:text-[#a1a1aa]">
                  <span>Status do Parecer:</span>
                  <span className={result.deterministic_approved ? 'text-emerald-600 dark:text-emerald-400 font-semibold' : 'text-rose-600 dark:text-rose-400 font-semibold'}>
                    {result.deterministic_approved ? 'Aprovado' : 'Reprovado'}
                  </span>
                </div>
                <div className="flex justify-between text-[#5e6270] dark:text-[#a1a1aa]">
                  <span>Status E-mail:</span>
                  <span className="text-[#4200db] dark:text-[#8575ff] font-semibold">{result.email_status}</span>
                </div>
                {result.email_result?.to && (
                  <div className="flex justify-between text-[#5e6270] dark:text-[#a1a1aa]">
                    <span>Destinatário:</span>
                    <span className="text-[#131920] dark:text-[#f4f4f5]">{Array.isArray(result.email_result.to) ? result.email_result.to.join(', ') : result.email_result.to}</span>
                  </div>
                )}
                <div className="flex justify-between text-[#5e6270] dark:text-[#a1a1aa]">
                  <span>Thread ID:</span>
                  <span className="text-[#8e92a0] dark:text-[#71717a] truncate max-w-[200px]">{result.audit_thread_id}</span>
                </div>
              </div>
            </div>
          )}
        ...
        {/* Footer */}
        <div className="p-4 sm:p-5 border-t border-[#e6e5f0] dark:border-[#262046] bg-[#f8f7fc] dark:bg-[#0d0b1a] flex items-center justify-end gap-2.5">
          {!result ? (
            <>
              <button
                onClick={onClose}
                disabled={loading}
                className="px-3.5 py-2 rounded-lg text-xs font-medium text-[#5e6270] dark:text-[#a1a1aa] hover:text-[#131920] dark:hover:text-[#f4f4f5] hover:bg-[#e6e5f0] dark:hover:bg-[#181530] transition-colors disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={loading}
                className="px-4 py-2 rounded-lg bg-[#4200db] hover:bg-[#35009e] text-[#ffffff] text-xs font-semibold flex items-center gap-2 transition-all shadow-md shadow-[#4200db]/20 active:scale-95 disabled:opacity-50"
              >
                {loading ? (
                  <>
            ...
            <>
              <button
                onClick={onClose}
                className="px-3.5 py-2 rounded-lg text-xs font-medium text-[#5e6270] dark:text-[#a1a1aa] hover:text-[#131920] dark:hover:text-[#f4f4f5] hover:bg-[#e6e5f0] dark:hover:bg-[#181530] transition-colors"
              >
                Fechar
              </button>
              {result.audit_thread_id && <button
                onClick={handleOpenCopilot}
                className="px-4 py-2 rounded-lg bg-[#4200db] hover:bg-[#35009e] text-[#ffffff] text-xs font-semibold flex items-center gap-1.5 transition-all shadow-md shadow-[#4200db]/20 active:scale-95"
              >
