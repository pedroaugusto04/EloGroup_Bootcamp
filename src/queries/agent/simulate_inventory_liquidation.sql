...
SELECT 
    ...
    COALESCE(c.preco_venda_medio, e.preco_venda_sugerido) AS preco_original,
    ROUND(COALESCE(c.preco_venda_medio, e.preco_venda_sugerido) * {discount_factor}, 2) AS preco_liquidacao,
    COALESCE(c.custo_medio_real, e.custo_unitario) AS custo_unitario,
    ROUND(e.estoque_disponivel * COALESCE(c.custo_medio_real, e.custo_unitario), 2) AS capital_travado_custo,
    ROUND(e.estoque_disponivel * COALESCE(c.preco_venda_medio, e.preco_venda_sugerido) * {discount_factor}, 2) AS receita_caixa_projetada,
    ROUND(
        (e.estoque_disponivel * COALESCE(c.preco_venda_medio, e.preco_venda_sugerido) * {discount_factor}) - 
        (e.estoque_disponivel * COALESCE(c.custo_medio_real, e.custo_unitario)), 2
