...
-- Vínculo CRM x ERP e pedidos anteriores ao cadastro
SELECT
...
-- Tickets citando pedidos ausentes/presentes no ERP
SELECT
...
-- Reconciliação dos campos históricos do CRM com o ERP (somente clientes com venda)
WITH erp AS (
    SELECT customer_id, COUNT(*) AS pedidos_erp, SUM(receita_liquida) AS receita_erp
    FROM vendas GROUP BY customer_id
)
SELECT COUNT(*) AS clientes_comparados,
       SUM(CASE WHEN c.total_pedidos_historico <> e.pedidos_erp THEN 1 ELSE 0 END) AS pedidos_divergentes,
       AVG(c.ltv_acumulado - e.receita_erp) AS diferenca_media_ltv_receita
FROM clientes c JOIN erp e USING (customer_id);
