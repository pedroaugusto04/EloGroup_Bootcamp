...
h1{{font-size:24px;line-height:1.2;margin:10px 0 18px;color:#18181b}} h2{{color:#2e0854;font-size:17px;margin:20px 0 10px}}
...
.card-capital{{background:#f7f5ff;border-top:3px solid #4200db}}.card-exposure{{background:#faf9ff;border-top:3px solid #8575ff}}
.card-liquidation{{background:#f4f0ff;border-top:3px solid #35009e}}.card-coverage{{background:#fcfcfd;border-top:3px solid #a99fff}}
...
<table class="metric-grid" role="presentation"><tbody>
  ...
  <tr>
    ...
    <td><div class="card card-coverage"><small>COBERTURA CADASTRAL E FOCO</small><h2>{capital.get("skus_com_custo_vendas", 0)} / {capital.get("total_skus", 0)} SKUs</h2><p>Categoria com maior atenção: {html.escape(str(top.get("categoria", "Não disponível")))}.</p></div></td>
  </tr>
</tbody></table>
<h2>SKUs Críticos com Margem em Risco</h2><table class="priorities"><thead><tr><th>SKU</th><th>Produto</th><th>Categoria</th><th>Margem em Risco</th></tr></thead><tbody>{rows}</tbody></table>
<h2>Plano de Ação · Quick Wins e Recomendações</h2>
<div class="decision-list">{decision_items}</div>
<p><small>Sinais: {operational.get('ruptura_atual', 0)} rupturas imediatas · {operational.get('ponto_pedido', 0)} no/abaixo do ponto de pedido. Cobertura de lead time com base cadastral.</small></p>
